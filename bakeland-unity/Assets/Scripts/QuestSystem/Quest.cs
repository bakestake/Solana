...
public class Quest
{
    ...
    public void InstantiateCurrentQuestStep(Transform parent)
    {
        ...
        {
            GameObject questStepObj = GameObject.Instantiate(questStepPrefab, parent);
            QuestStep questStep = questStepObj.GetComponent<QuestStep>();
            if (questStep != null)
            {
                // Initialize with both the quest ID and the saved state
                questStep.Initialize(info.id, currentQuestStepIndex);
                questStep.LoadState(questStepStates[currentQuestStepIndex]);
                Debug.Log($"Instantiated quest step {currentQuestStepIndex} for quest {info.id}");
            }
        }
    }
