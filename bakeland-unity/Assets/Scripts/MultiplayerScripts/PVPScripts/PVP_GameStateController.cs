...
public class PVP_GameStateController : NetworkBehaviour
{
    enum GameState
    {
        Starting,
    ...
    public override void Spawned()
    {
        ...
        if (_gameState != GameState.Starting)
        {
            foreach (var player in Runner.ActivePlayers)
            {
                ...
                TrackNewPlayer(playerObject.GetComponent<Health_Playerr>().Id);
            }
        }
    ...
    private void UpdateStartingDisplay()
    {
        ...
        FindObjectOfType<PVP_PlayerSpawner>().StartplayerSpawner(this);
    ...
    private void UpdateEndingDisplay()
    {
        ...
        if (Runner.TryFindBehaviour(_winner, out Health_Playerr playerData) == false) return;

        _startEndDisplay.gameObject.SetActive(true);
        //_ingameTimerDisplay.gameObject.SetActive(false);
        _startEndDisplay.text =
            $"{playerData.NickName} won with {playerData.currentHealth} points. Disconnecting in {Mathf.RoundToInt(_timer.RemainingTime(Runner) ?? 0)}";
    ...
    public void CheckIfGameHasEnded()
    {
        ...
        for (int i = 0; i < _playerDataNetworkedIds.Count; i++)
        {
            if (Runner.TryFindBehaviour(_playerDataNetworkedIds[i],
                    out Health_Playerr playerDataNetworkedComponent) == false)
            {
            ...
            if (playerDataNetworkedComponent.currentHealth > 0) playersAlive++;
        }
        ...
        foreach (var playerDataNetworkedId in _playerDataNetworkedIds)
        {
            if (Runner.TryFindBehaviour(playerDataNetworkedId,
                    out Health_Playerr playerDataNetworkedComponent) ==
                false) continue;

            if (playerDataNetworkedComponent.currentHealth > 0 == false) continue;
