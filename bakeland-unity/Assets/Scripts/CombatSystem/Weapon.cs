...
public class Weapon : MonoBehaviour
{
    ...
    public float attackSwingDistance = 0.5f;
    ...
    private void ThrustWeapon(TweenCallback onComplete)
    {
        ...
        hitboxVisual.StartWaveEffect(attackDirection);
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (((1 << collision.gameObject.layer) & enemyLayer) != 0)
        {
            if (!hitEnemies.Contains(collision.gameObject))
            {
                ...
                collision.gameObject.GetComponent<Health>().TakeDamage(weaponData.damage, player.position);
