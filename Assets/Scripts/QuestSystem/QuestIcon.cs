    ...
    public void SetState(QuestState newState, bool startPoint, bool finishPoint)
    {
        ...
        switch (newState)
        {
            case QuestState.REQUIREMENTS_NOT_MET:
                if (startPoint || (!startPoint && !finishPoint)) { requirementsNotMetToStartIcon.SetActive(true); }
                break;
            case QuestState.CAN_START:
                if (startPoint || (!startPoint && !finishPoint)) { canStartIcon.SetActive(true); }
                break;
            case QuestState.IN_PROGRESS:
                if (finishPoint || (!startPoint && !finishPoint)) { requirementsNotMetToFinishIcon.SetActive(true); }
                break;
            case QuestState.CAN_FINISH:
                if (finishPoint || (!startPoint && !finishPoint)) { canFinishIcon.SetActive(true); }
                break;
